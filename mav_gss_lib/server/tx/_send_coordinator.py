...
class _SendCoordinator:
    ...
    async def _wait_for_pending_verifications_clear(
        ...
        """Block until every open verifier instance is settled, or abort.

        At most one verification window is open at a time. The next
        mission_cmd waits until every prior instance is settled (terminal
        stage — see ``is_settled``) before publishing. Cross-target batches
        serialize too — accepted tradeoff for predictable per-row UI.

        Hard cap of `max_wait_ms` prevents the queue from stalling forever
        if the periodic sweeper somehow fails to drive a timed_out instance
        to terminal; proceed with the next send rather than freeze.
    ...
    async def run_send(self) -> None:
        ...
        try:
            ...
            # command's verifier instance settles (complete / failed /
